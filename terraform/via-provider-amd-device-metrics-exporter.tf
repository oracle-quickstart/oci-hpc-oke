...
resource "helm_release" "amd_device_metrics_exporter" {
  count             = alltrue([var.create_cluster, var.install_monitoring, var.install_amd_device_metrics_exporter && (var.worker_rdma_shape == "BM.GPU.MI300X.8" || var.worker_gpu_shape == "BM.GPU.MI300X.8"), var.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? 1 : 0
