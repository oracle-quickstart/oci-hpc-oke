...
resource "helm_release" "nvidia_dcgm_exporter" {
  count             = alltrue([var.install_monitoring, var.install_nvidia_dcgm_exporter, var.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? 1 : 0
