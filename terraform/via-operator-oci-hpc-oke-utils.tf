...
module "oci_hpc_oke_utils" {
  count  = alltrue([var.install_oci_hpc_oke_utils, anytrue([var.worker_rdma_enabled, var.worker_gmc_enabled, local.slinky_hostname_annotator_enabled]), local.deploy_from_operator]) ? 1 : 0
  source = "./helm-module"
