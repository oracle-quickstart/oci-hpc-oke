...
variable "subnets" {
  ...
  validation {
    condition = alltrue([
      for v in flatten([for k, v in var.subnets : keys(v)]) : contains(["create", "id", "cidr", "ipv4cidr_blocks", "netnum", "newbits", "display_name", "dns_label", "ipv6_cidr", "secondary_vnic_pod_subnet"], v)
    ])
    error_message = format("Invalid subnet configuration keys: %s", jsonencode(distinct([
      for v in flatten([for k, v in var.subnets : keys(v)]) : v if !contains(["create", "id", "cidr", "ipv4cidr_blocks", "netnum", "newbits", "display_name", "dns_label", "ipv6_cidr", "secondary_vnic_pod_subnet"], v)
    ])))
  }
}
