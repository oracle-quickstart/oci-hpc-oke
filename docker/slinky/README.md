...
The `1.2.1` Slurm chart defaults to upstream `26.05-ubuntu26.04` images, so the
...
(chart `1.2.1` supports Slurm `25.11` and later).
...
| Role | Image |
| --- | --- |
| Operator manager | `ghcr.io/slinkyproject/slurm-operator:1.2.1` |
| Operator webhook | `ghcr.io/slinkyproject/slurm-operator-webhook:1.2.1` |
...
- `operator` and `webhook` build from a pinned clone of
  `SlinkyProject/slurm-operator` @ `v1.2.1` (its Dockerfile needs the Go source
...
| Slinky operator | `slurm-operator` target `manager` | `iad.ocir.io/idxzjcdglx2s/slurm-operator:1.2.1` | `linux/amd64`, `linux/arm64` |
| Slinky operator webhook | `slurm-operator` target `webhook` | `iad.ocir.io/idxzjcdglx2s/slurm-operator-webhook:1.2.1` | `linux/amd64`, `linux/arm64` |
