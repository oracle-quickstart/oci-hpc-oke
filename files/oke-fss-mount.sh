...
# Mount the NFS share
mount -t nfs -o vers=3,nconnect=16 "$MOUNT_TARGET_IP:$EXPORT_PATH" "$MOUNT_POINT"
if [ $? -eq 0 ]; then
  echo "Successfully mounted $MOUNT_TARGET_IP:$EXPORT_PATH at $MOUNT_POINT"
else
  echo 'Error mounting NFS volume' >&2
  exit 1
fi

# Add entry to /etc/fstab for re-mount at boot (if not already present)
if ! grep -q "$MOUNT_TARGET_IP:$EXPORT_PATH" /etc/fstab; then
  echo "$MOUNT_TARGET_IP:$EXPORT_PATH $MOUNT_POINT nfs vers=3,nconnect=16,_netdev 0 0" >> /etc/fstab
