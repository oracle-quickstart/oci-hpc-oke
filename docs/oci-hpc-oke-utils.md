...
The labeler and prepuller target GPU nodes by default (nodes with `nvidia.com/gpu` or `amd.com/gpu` labels). Hostexec has no node selector and runs on all nodes.
